...
//     SMQ_ASSERT(((role < SMQ_ROLE_MIN) || (role > SMQ_ROLE_MAX)), "对象角色不支持");
    ...
    smq_memset(NULL, 0, sizeof(smq_t));

     smq_uint32 shm_size = smq_params.memory_size * 1024 * 1024;
//     smq_shm_t* shm = NULL;
//     smq_errno err = smq_shm_open(name, shm_size, &shm);
...
// 
//     smq_errno err = 0;//smq_shm_open(name, , &(smq->shm));
//     if (SMQ_OK != err)
//     {
